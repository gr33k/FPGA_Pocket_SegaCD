...
cleanup_generated_outputs() {
  ...
  for d in "${clean_generated_dirs[@]}"; do
    if [[ -d "$UPSTREAM_DIR/$d" ]]; then
      rm -rf "$UPSTREAM_DIR/$d"
      clean_generated_items+=("$UPSTREAM_DIR/$d")
      removed=1
    fi
  done

  for f in "$UPSTREAM_DIR"/*.{sof,pof,jic,rpd,rbf,rbf_r}; do
    [[ -f "$f" ]] || continue
    rm -f "$f"
  ...
  if [[ "$removed" -eq 1 ]]; then
    append_status "Generated-output warning: cleanup removed Quartus artifacts from upstream project dir."
...
if [[ -n "$selected_quartus_map" && -x "$selected_quartus_map" ]]; then
  ...
  if (cd "$UPSTREAM_DIR" && "${command_line[@]}" > "$LOG_PATH" 2>&1); then
